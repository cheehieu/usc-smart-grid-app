...
class GNUPlot {
    ...
    function formatPlot( ) {
        $this->exe( "set terminal png truecolor\n");
        $this->exe( "set terminal png xC9C9C9\n");
        $this->exe( "set grid\n");
        $this->exe( "set style data histograms\n");
        $this->exe( "set style fill solid 1.0 border -1\n");
        $this->exe( "set xdata time\n");
        $this->exe( "set timefmt '%H:%M'\n");
	$this->exe( "set format x '%H:%M'\n");
	$this->exe( "set format y '%4.2f'\n");
	$this->exe( "set tics out\n");
	$this->exe( "unset key\n");
    }
    ...
    function plotData(  &$PGData, $method, $using, $axis='', $extra='' ) {
        ...
        $this->exe( "$plot '<sort -n -k 1 $fn' using $using title \"$title\" with $method  $axis $extra\n");
